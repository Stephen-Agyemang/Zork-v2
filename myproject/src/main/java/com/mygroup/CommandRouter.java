...
public class CommandRouter {
    ...
    public CommandRouter(GameState state, FoodSystem foodSystem, TypingChallengeSystem typingChallengeSystem, DNASystem dnaSystem, QuestSystem questSystem) {
        this.state = state;
        ...
        switch (command) {
            ...
            case "take" -> {
                if (words.length < 2) {
                    ...
                    String itemName = String.join(" ", Arrays.copyOfRange(words, 1, words.length));
                    // Special-case: allow picking up the sealed TreasureBox to carry it safely to Olin
                    if (state.getCurrLocation().hasContainerItem(itemName)) {
                        ...
                        for (int i = 1; i < words.length; i++) {
                            if (words[i].equals("from")) { fromIndex = i; break; }
                        }
                        if (fromIndex > 1 && fromIndex < words.length - 1) {
                            ...
                            // Dangerous action: trying to remove snakes from the box costs points and is disallowed
                    ...
                    if (state.getCurrLocation().hasItem(itemName)) {
                        ...
                        // Check if they're getting food from the second dining location (triggers dangerous bonus potential)
                        if (foodSystem.isFoodItem(taken)) {
                            foodSystem.markFoodFromSecondDining();
                        }
                        
                        // Phone triggers Sasha call
                        if (Item.normalizeName(taken.getName()).equals(Item.normalizeName("Phone")) && !state.isSashaCalled()) {
                            state.callSasha();
            ...
            case "drop" -> {
                ...
                if (Item.normalizeName(itemName).contains("batman's-d") && state.getCurrLocation().getName().equalsIgnoreCase("Julian") && !state.isDnaTaskComplete()) {
                    state.completeDNATask();
                    state.addPoints(15);
                    return "You dropped " + itemName + ". The lab tech secures the sample. DNA delivery complete! +15 points." + questSystem.maybeFinaleMessage();
                }
            ...
            case "put" -> {
                ...
                if (containerName.equalsIgnoreCase("Aquarium") && Item.normalizeName(moving.getName()).equals(Item.normalizeName("EndangeredSalmon")) && !state.isSalmonTaskComplete()) {
                ...
                if (containerName.equalsIgnoreCase("SafeBox") && Item.normalizeName(moving.getName()).equals(Item.normalizeName("TreasureBox")) && !state.isSnakeTaskComplete()) {
                ...
                if (containerName.equalsIgnoreCase("MacbookCase") && Item.normalizeName(moving.getName()).equals(Item.normalizeName("Macbook")) && !state.isMacbookTaskComplete()) {
                ...
                if (Item.normalizeName(moving.getName()).contains("batman's-d") && state.getCurrLocation().getName().equalsIgnoreCase("Julian") && !state.isDnaTaskComplete()) {
                    state.completeDNATask();
                    state.addPoints(15);
            ...
            case "help" -> {
                if (state.getInventory().hasItem("Help")) {
                    ...
                    return "You dropped your Help guide at " + state.getHelpDropLocation().getName() + "! Go back there to find it and examine it.";
                } else {
